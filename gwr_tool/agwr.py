...
class AssociativeGWR:
    ...
    def compute_distance(self, x, y, m) -> float:       #欧氏距离 or 余弦距离
        return np.linalg.norm(x-y) if m else scipy.spatial.distance.cosine(x, y)

    def find_bs(self, dis) -> Tuple[int, float, int]:
        bs = nsmallest(2, ((k, i) for i, k in enumerate(dis)))   #找出距离最小的两组
        return bs[0][1], bs[0][0], bs[1][1]    #最小距离编号，最小距离值，次最小距离编号
    ...
    def init_network(self, ds, random) -> None:
        ...
        self.num_nodes = 2
        self.dimension = ds.vectors.shape[1]      #特征数
        empty_weight = np.zeros(self.dimension)
        self.weights = [empty_weight, empty_weight]   #所有节点的权重
        ...
        self.alabels = np.stack([empty_label_hist,empty_label_hist],axis=0).tolist()
        ...
        for i in range(0, len(init_ind)):
            ...
            self.alabels[i][int(ds.labels[init_ind[i]])] = 1                            #！！！！！ !!!!!!!error code
