...
@jit(nopython=True)
def compute1_distance(al,weights,y) -> float:
    distances = np.zeros(len(weights))
    for i,wei in enumerate(weights):
        distances[i] = np.linalg.norm(np.dot(al.T, (wei - y)))
    return distances

def compute2(args):
    al, weight, input, ind = args
    return (np.linalg.norm(np.dot(al.T, (weight - input))),ind)
...
class GammaGWR:
    ...
    def find_bs(self, dis) -> Tuple[int, float, int]:
        bs = nsmallest(2, [(k,d) for d,k in enumerate(dis)])
        # bs = nsmallest(2, dis)
        return bs[0][1], bs[0][0], bs[1][1]


    def find_bmus(self, input_vector,train=True, **kwargs) -> Union[Callable[[np.ndarray], Any], Tuple[int, float]]:
        second_best = kwargs.get('s_best', False)
        distances = np.zeros(self.num_nodes)
        if self.trained_nodes == None or not train:
            for i in range(0, self.num_nodes):
                distances[i] = self.compute_distance(self.weights[i],input_vector)
        else:
            for i in range(self.trained_nodes-2):
                distances[i] = 10000
            for i in range(self.trained_nodes-2,self.num_nodes):
                distances[i] = self.compute_distance(self.weights[i], input_vector)
    ...
    def remove_isolated_nodes(self) -> None:
        if self.num_nodes > 2:
            ...
            while (ind_c < self.num_nodes):
                ...
                if self.train_one == 1:
                    if ind_c >= len(self.consoli_nodes):
                        ...
                        ind_c += 1
                    # elif self.consoli_nodes[ind_c] == 1:
                    #     ind_c += 1
                    # elif self.consoli_nodes[ind_c] == 0:
                    #     if len(neighbours[0]) < 1:
                    #         self.weights.pop(ind_c)
                    #         self.alabels.pop(ind_c)
                    #         self.habn.pop(ind_c)
                    ...
                    #         self.ages = np.delete(self.ages, ind_c, axis=1)
                    #         self.num_nodes -= 1
                    #         self.consoli_nodes = np.delete(self.consoli_nodes,ind_c)
                    #         rem_c += 1
                    #     else:
                    #         ind_c += 1
                else:
