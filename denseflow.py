...
def save_flows(flows,vd_name,image,save_dir,num,bound):   #flowDTVL1,image,save_dir,frame_num,bound
    '''
    ...
    class_dir = vd_name.split('_')[1].split('.')[0]

    if not os.path.exists(os.path.join(data_root,new_dir_frame,class_dir,save_dir)):
        os.makedirs(os.path.join(data_root,new_dir_frame,class_dir,save_dir))
    if not os.path.exists(os.path.join(data_root,new_dir_flow,class_dir,save_dir,'flow_x')):
        os.makedirs(os.path.join(data_root,new_dir_flow,class_dir,save_dir,'flow_x'))
    if not os.path.exists(os.path.join(data_root,new_dir_flow,class_dir,save_dir,'flow_y')):
        os.makedirs(os.path.join(data_root,new_dir_flow,class_dir,save_dir,'flow_y'))

    #save the image
    save_img=os.path.join(data_root,new_dir_frame,class_dir,save_dir,'img_{:05d}.jpg'.format(num))
    scipy.misc.imsave(save_img,image)

    #save the flows
    save_x=os.path.join(data_root,new_dir_flow,class_dir,save_dir,'flow_x','flow_x_{:05d}.jpg'.format(num))
    save_y=os.path.join(data_root,new_dir_flow,class_dir,save_dir,'flow_y','flow_y_{:05d}.jpg'.format(num))
...
def dense_flow(augs):     #zip(video_list,flows_dirs,[step]*len(video_list),[bound]*len(video_list))
    '''
    ...
    video_path=os.path.join(videos_root,video_name.split('_')[1].split('.')[0],video_name)
    ...
    while True:
        ...
        save_flows(flowDTVL1,video_name,image,save_dir,frame_num,bound) #this is to save flows and img.
        prev_gray=gray
        prev_image=image
        frame_num+=1
        # to pass the out of stepped frames
...
def parse_args():
    ...
    parser.add_argument('--e_',default=600,type=int,help='end id')
...
if __name__ =='__main__':
    ...
    #mjw
    videos_root = '/data1/ma_gps/new_kth/videos/'
    data_root = '/data1/ma_gps/new_kth/'


    #specify the augments
    num_workers=args.num_workers
    step=args.step    #frames gap = 1
    bound=args.bound  #15 for optical flow
    s_=args.s_        #start id  0
    e_=args.e_        #end id   600 for KTH_dataset
    new_dir=args.new_dir
    new_dir_flow = 'flows'
    new_dir_frame = 'frames'
    mode=args.mode
    #get video list
    video_list,len_videos=get_video_list()  #the all filename
